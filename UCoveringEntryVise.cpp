...
class RoomatesProblemSol : public RoomatesProblemGen
{
	public:
        ...
        bool CFiveNonUCoveringOnlyFive(bool write) {
			int exatly_five = 0;
			for(const auto &v : edge_list_) 
				if((int)v.size() == vertex_numb_-5 ) 
					++exatly_five;
			...
			for(int k = 0; k < vertex_numb_; ++k)  {
				const vector<int> UKs{k};
				if((int)edge_list_[ UKs[0] ].size() >= vertex_numb_-5) 
					if(MultipleNonUCoverPopular(UKs, write))   
						return true;
			}
		...
		void PopularMaximalBF2(vector<int> &Pairs, int &depth, bool &end) {
			if(end) return;
			if(depth == (int)Pairs.size()) {
				ListGraph g;
				std::vector<lemon::ListGraph::Node> nodes;
...
int WhoseDeg(RoomatesProblemSol &Curr) {
    int LowestDegrreN3 = -1;
    for(int i = 6; i >= 0; --i)  {
        for(int k = 0; k < Curr.vertex_numb_; ++k) {
            if((int)Curr.edge_list_[ k ].size() == Curr.vertex_numb_-i) {LowestDegrreN3 = i; }
        }
        if(LowestDegrreN3 != -1) {
            return i;
        }
...
int main() {
    ...
    int testNumber;
    if(QUESTIONS)  cout << "Seventh: How many Test should i run now?\n";
    cin >> testNumber;
    ...
    for(int vertexNumb = N; vertexNumb < End+Steper; vertexNumb += Steper) {
        for(int lowestDeg = 3; lowestDeg <= cdeg; ++lowestDeg) {
			auto start = chrono::steady_clock::now();
            int i = 0;
            while(trialsdegree < testNumber) {
                RoomatesProblemSol Test(vertexNumb);
                switch(caser) {
                    case 1: Test.ErdosRenyi(1.0-double(vertexNumb-lowestDeg+1)/(double(vertexNumb-1))); break;
                    //case 1: Test.ErdosRenyi(); break;//Test.DegreeAdder(cdeg); break;
                    case 2: Test.ErdosRenyi(1.0/2.0-double(vertexNumb-lowestDeg)/(2*double(vertexNumb-1))); break;
                    //case 2: Test.HavelHakimi(cdeg); break;
                    case 3: Test.PruferGraphCr(); Test.DegreeAdder(lowestDeg) ; break;
                }
                switch(prefmodel) {
                ...
                for(int k = 0; k < vertexNumb; ++k) {
                    if((int)Test.edge_list_[ k ].size() == vertexNumb-lowestDeg) LowestDegrreN3 = true;
                }
                
                
                bool min_degree = true;
                for(int k = 0; k < vertexNumb; ++k) {
                    if((int)Test.edge_list_[ k ].size() < vertexNumb-lowestDeg) min_degree = false;
                }
                //int smallest_deg = WhoseDeg(Test); //cout << smallest_deg << endl;
                if(min_degree && LowestDegrreN3) {
                    if(!Test.IrvingStableBool()) {
                        ++trialsinner;
                        bool PopExistsU = Test.CFiveNonUCovering(false);
						//bool PopExistsU = Test.PopularBruteForce2();
                        if(PopExistsU) {++i;}
                    }
                    ++trialsdegree;
                }
            ...
            cout << "Vertex Number: " << vertexNumb << endl;
            cout << "C deg Number: " << lowestDeg << endl;
            ...
            cout << "Wasted: " << waste << endl;
            cout << endl << endl;
            sumofStableMatchs = 0;
            trialsinner = 0; trialsoutest = 0; trialsdegree = 0;
            waste = 0;
			auto end = chrono::steady_clock::now();
			cout << "ELAPSE TIME: " << chrono::duration_cast<chrono::milliseconds>(end - start).count() << endl << endl;
        }
    }
