...
int main() {
    ...
    for(int vertexNumb = N; vertexNumb < End+Steper; vertexNumb += Steper) {
        ...
        while(trialsdegree < testNumber) {
			...
			switch(caser) {
				case 1: Test.ErdosRenyi(1.0/2.0-double(vertexNumb-cdeg)/(2*double(vertexNumb-1))); break;
            ...
            */
            ...
            for(int k = 0; k < vertexNumb; ++k) {
                if((int)Test.edge_list_[ k ].size() < vertexNumb-5) min_degree = false;
            }
			...
			if(min_degree) {
				if(!Test.IrvingStableBool()) {
					...
					if(PopExistsU) {++i; ++DegreVar[smallest_deg]; }
				}
				++trialsdegree;
			}
			++trialsoutest;
        ...
        cout << "By Degrees" << endl; Print_vector(DegreVar);
        sumofStableMatchs = 0;
